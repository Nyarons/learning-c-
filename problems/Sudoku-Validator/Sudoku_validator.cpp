...
void sudoku_validator(vector<vector<int>> sudoku_arr);
int main()
{
...
void sudoku_validator(vector<vector<int>> sudoku_arr) {
	...
	for (int i = 0; i < sudoku_arr.size(); i++) {
		...
		if (!isValidLine(block)) show_line(block, "box");
		if (!isValidLine(horizontal)) show_line(horizontal,"horizontal");
		if (!isValidLine(vertical))   show_line(vertical,"vertical");
	}
